...
- Workflowware definition;
- Agentic Workflow Distribution definition;
- minimum package structure;
- README template;
- workflow-spec.md template;
- agent-spec.md template;
- approval-rules.md template;
- eval-template.csv example;
...
> Give solo operators, creators, and consultants a packaged set of templates they can use immediately.
...
> Turn one messy recurring workflow into a clear Workflowware plan.

Deliverables:

- old-way/new-way workflow map;
- agent opportunity map;
- risk and approval notes;
- package file outline;
...
> Build a real package for one workflow.
...
> Install a governed workflow system backed by BlackLabRats + Hermes + MAAT Runtime.

Includes:

- client-specific Workflowware package;
- eval set;
- approval workflow;
- memory/artifact registry;
...
## Tier 5 — Training / installer program
...
> Teach freelancers, consultants, or small agencies how to package, sell, and install Workflowware.

Includes:

- training curriculum;
- example workflow kits;
- sales scripts;
- intake forms;
- eval templates;
- approval frameworks;
...
- MAAT-backed registry/memory;
- governance logs;
- reusable private package library;
- installer training;
